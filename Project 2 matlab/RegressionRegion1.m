...
Xr = [arr_FF_table(:, 6) arr_FF_table(:, 7) log(arr_FF_table(:, 8)+1)];
yReg1=y(1:122,:);
XrReg1=Xr(1:122,:);
%y = normalize(arr_FF_table(:, 5));
%% Regression, part a

%% 1
%Regression of temperature using humidity, rain, wind

w_est = glmfit(XrReg1, yReg1, 'normal');
y_est = glmval(w_est, XrReg1, 'identity');
...
mfig('Residual error');
histogram(yReg1-y_est, 40);

%% Normalized Xr


Xr_norm=normalize(XrReg1);%(Xr - mean(Xr))./std(Xr);
X=Xr_norm;
w_est = glmfit(X, yReg1, 'normal');
